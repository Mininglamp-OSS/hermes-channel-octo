...
# Inbound file handling (mirrors openclaw inbound.ts).
...
class OctoAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig) -> None:
        ...
        # racing each other into self-kick storms (#139 in openclaw). Cleared
        ...
        # humans, not to fan-out to every assistant. Mirrors openclaw
        # ignoreMentionAll.
        ...
        # default when not set. Mirrors openclaw historyPromptTemplate.
    ...
    def _build_member_list_prefix(self) -> str:
        ...
        Mirrors openclaw inbound.ts :: buildMemberListPrefix.
        """
    ...
    async def _handle_recv(self, recv: Any) -> None:
        ...
        # pipeline hanging on slow remote URLs — mirrors openclaw). The
    ...
    async def _download_inbound_media_to_local(
        ...
        trick openclaw uses. Capped at MEDIA_DOWNLOAD_MAX_BYTES; oversize
    ...
    def _resolve_inner_message_text(self, inner: dict[str, Any]) -> str:
        ...
        Mirrors openclaw's resolveInnerMessageText. Each inner ``msg`` has a
        ``payload`` block with the same ``type``/``content``/``url``/``name``
        shape as a top-level message; we reuse the same placeholders so the
        LLM sees consistent labels.
        """
    ...
    def _resolve_multiple_forward_text(self, payload: Any) -> str:
        ...
        Mirrors openclaw's resolveMultipleForwardText: each inner message is
        rendered as ``<senderName>: <body>`` (sender resolved from the
        forward's own ``users[]`` array first, falling back to the uid). The
        ``users`` list is also used to opportunistically fill the long-lived
        uid→name map, so downstream display benefits even after the forward
        message itself is processed.
    ...
    async def _schedule_reconnect(self) -> None:
        ...
        # storm (#139 in openclaw-channel-octo).
