...
Strict translation from openclaw-channel-octo/src/socket.ts.
...
Security note: AES-128-CBC without HMAC and the MD5-based KDF below are
fixed by the WuKongIM v4 wire format and cannot be unilaterally upgraded
without coordinating a server-side protocol bump.
"""
...
def try_unpack_one(buf: bytearray) -> tuple[bytes | None, bytearray]:
    """
