...
name: octo-bot-api
version: 0.1.0
description: Octo Bot API 文档。消息发送、群管理、Thread、文件上传、User API 等接口。API 基础地址通过环境变量 OCTO_API_URL 注入。
...
## ⚠️ Agent Tool Selection (Hermes Runtime)
...
octo_management(
  action="send-message",
  target="<user_uid> | <group_no> | <group_no>____<short_id> | <group_no>:<short_id>",
  content="...",
  reply_to_message_id="<optional>",
  mention_uids=["uid1", "uid2"],   # optional
  mention_all=False                 # optional
)
```
...
### Recommended: hermes-channel-octo plugin

Install the pre-built adapter for instant message delivery, real-time online
status, and auto-reconnect. The plugin registers itself with hermes-agent via
Python entry-points and starts listening as soon as the gateway boots.

```bash
# From GitHub (until PyPI release):
$HERMES_HOME/.venv/bin/pip install \
    'git+https://github.com/Mininglamp-OSS/hermes-channel-octo.git'
...
**Note (Space mode):** In Space-enabled deployments, the underlying WuKongIM channel_id uses `s{spaceId}_{uid}` format. The bundled adapter handles this automatically. If you use the events API directly, `from_uid` remains the bare UID — use it as-is for sendMessage.
